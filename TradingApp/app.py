...
def process(raw: pd.DataFrame, dd_buf: float, profit_target: float) -> dict:
    """
    Parse CSV, split out cash-flows, compute cumulative PnL from $0.
    Returns a dict: df, cashflows, total_cashflow.
    """
    df = raw.copy()
    ...
    # Separate cash-flow rows before any math
    ...
    # Parse PnL (gross from broker — no manual fee deduction)
    df["gross_pnl"] = df["pnl"].apply(parse_pnl)

    # Cumulative PnL from $0
    df["cum_pnl"] = df["gross_pnl"].cumsum()

    # Trailing drawdown line (relative to $0 baseline)
    ...
    # Monotone timestamp for charting
    df["ts_plot"] = smooth_ts(df["soldTimestamp"])

    return {"df": df, "cashflows": cf_df, "total_cf": total_cf}


@st.cache_data(show_spinner=False)
def load_csv(data: bytes, dd_buf: float, profit_target: float) -> dict:
    raw = pd.read_csv(io.BytesIO(data))
    return process(raw, dd_buf=dd_buf, profit_target=profit_target)
...
with st.sidebar:
    ...
    st.markdown(_sec + "Account Anchor</div>", unsafe_allow_html=True)

    broker_equity = st.number_input(
        "Current Tradovate Equity ($)",
        min_value=0.0,
        value=50_000.0,
        step=100.0,
        format="%.2f",
        help="Paste your exact live equity from Tradovate. "
             "Used only for the top metric card — chart tracks cumulative PnL from $0.",
    )
...
#  RESOLVE DATA
# ─────────────────────────────────────────────────────────────────────────────
_proc_args = (dd_buffer, profit_target)
...
total_pnl   = float(df["gross_pnl"].sum())   # sum of broker-reported PnL
...
avg_win     = float(wins["gross_pnl"].mean())   if len(wins)   else 0.0
avg_loss    = float(losses["gross_pnl"].mean()) if len(losses) else 0.0
rr          = (avg_win / abs(avg_loss))          if avg_loss    else float("inf")
_mdd        = max_drawdown(df["cum_pnl"])
_dfp        = dist_from_peak(df["cum_pnl"])
win_st, loss_st = streak_calc(df["gross_pnl"])
...
# Risk of ruin — how many consecutive losses until drawdown breached
...
if active_tab == "Dashboard":
    ...
    # Current Equity = broker-input anchor (not calculated)
    k1.metric(
        "Current Equity",
        f"${broker_equity:,.2f}",
        f"Broker-anchored value",
        delta_color="off",
    )
    k2.metric(
        "Gross Profit",
    ...
    k7.metric(
        ...
        fmt_usd(-_mdd),
        f"−{_mdd/broker_equity*100:.1f}% of equity",
        delta_color="inverse",
    )
    k8.metric(
        "Distance from Peak",
    ...
    eq = pd.concat(
        [origin, df[["ts_plot", "cum_pnl", "dd_limit", "trade_num", "gross_pnl"]]],
        ignore_index=True,
    )
    ...
    fig_eq.add_trace(go.Scatter(
        ...
        hovertemplate=(
            ...
            "%{x|%b %d  %H:%M}<br>"
            "Cum. P&L: <b>$%{y:,.2f}</b><br>"
            "Trade: $%{customdata[1]:,.2f}"
            "<extra></extra>"
        ),
        customdata=np.column_stack([
            eq["trade_num"].to_numpy(),
            eq["gross_pnl"].to_numpy(),
        ]),
    ))
    ...
    with al:
        ...
        dow = (df.groupby("day_of_week")["gross_pnl"]
                 .sum()
    ...
    with c1:
        ...
        hourly = (df.groupby("hour")["gross_pnl"]
                    .sum()
    ...
    with c2:
        ...
        ls = (df.groupby("direction")["gross_pnl"]
    ...
    # Monthly bar
    monthly = (df.groupby("month_label")["gross_pnl"]
                 .sum()
                 .reset_index()
                 .rename(columns={"month_label": "m", "gross_pnl": "pnl"}))
    if len(monthly) > 1:
    ...
    _pnl_num = tbl["P&L"].copy()
    _dir_ref = tbl["Dir"].copy()
    tbl["P&L"] = _pnl_num.apply(lambda v: f"+${v:,.2f}" if v > 0 else f"-${abs(v):,.2f}")
    ...
    lw = f"${float(wins['gross_pnl'].max()):,.2f}"   if len(wins)   else "—"
    ll = f"${float(losses['gross_pnl'].min()):,.2f}" if len(losses) else "—"

    _pill(p1, "Gross Profit",  f"${gross_wins:,.2f}")
    _pill(p2, "Gross Loss",    f"${gross_loss:,.2f}")
    _pill(p3, "Largest Win",   lw)
    _pill(p4, "Largest Loss",  ll)
    _pill(p5, "Expectancy",    f"${expectancy:,.2f}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with pc3:
        start_eq = st.number_input(
            "Starting Equity ($)",
            min_value=0.0,
            value=float(broker_equity),
            step=500.0,
            format="%.2f",
            help="Defaults to your broker equity anchor.",
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    day_pnl   = float(day_df["gross_pnl"].sum()) if len(day_df) else 0.0
    day_wins  = int((day_df["gross_pnl"] > 0).sum())
    day_losses= int((day_df["gross_pnl"] < 0).sum())
    ...
    with jr:
        ...
        if len(day_df) == 0:
            ...
            dt = day_df[["trade_num","symbol","direction","qty","gross_pnl","duration"]].copy()
            dt.columns = ["#","Symbol","Dir","Qty","P&L","Dur"]
            _nr = dt["P&L"].copy()
            _dr = dt["Dir"].copy()
            dt["P&L"] = _nr.apply(lambda v: f"+${v:,.2f}" if v > 0 else f"-${abs(v):,.2f}")
            sdt = (
                dt.style
                .apply(lambda col: [
