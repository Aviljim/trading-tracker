...
#  RULE: PLOT dict contains ONLY keys that are NEVER passed explicitly.
#  xaxis / yaxis are NOT in PLOT — they are built per-chart using _axis() below.
# ─────────────────────────────────────────────────────────────────────────────

# Shared base — no xaxis/yaxis/margin so they can always be passed explicitly
PLOT = dict(
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="rgba(0,0,0,0)",
    font=dict(family="DM Sans, sans-serif", color="#8888aa", size=12),
    hoverlabel=dict(
        bgcolor="#1c1c28", bordercolor="#2e2e45",
        font=dict(family="DM Mono, monospace", size=12, color="#f0f0fa"),
    ),
)

def _axis(tickprefix="", tickformat=",", zeroline=False,
          zerolinecolor="rgba(255,255,255,0.10)", extra=None):
    """Return a standard dark-theme axis dict. Merge extra kwargs if needed."""
    d = dict(
        showgrid=True,
...
def calc_fees(symbol: str, qty) -> float:
    """
    Per-contract fee estimate:
      MNQ → $1.14/contract
      NQ  → $4.14/contract  (must check MNQ first — MNQ contains NQ)
      All others → $0.00
    """
...
def calc_trailing_drawdown(cum_net_pnl: pd.Series, buffer: float = 2000.0) -> pd.Series:
    ...
    Trailing drawdown line for a funded account:
      - Starts at -buffer
      - Trails up with every new High Water Mark
      - Never drops when PnL falls
      - Permanently locks at $0 once HWM reaches buffer
      Formula: limit = clip(HWM - buffer, upper=0)
    """
    hwm = cum_net_pnl.cummax()
    return (hwm - buffer).clip(upper=0)
...
def max_drawdown_from_series(cum_pnl: pd.Series) -> float:
    """Peak-to-trough max drawdown (returned as positive number)."""
    peak = cum_pnl.cummax()
    return float(abs((cum_pnl - peak).min()))
...
def generate_demo() -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        win = rng.random() < 0.56
        pnl = round(
            float(abs(rng.normal(350, 280))) if win
...
def process(raw: pd.DataFrame, starting: float, dd_buffer: float = 2000.0) -> pd.DataFrame:
    df = raw.copy()

    df["gross_pnl"] = df["pnl"].apply(parse_pnl)
    df["fees"] = df.apply(lambda r: calc_fees(r["symbol"], r.get("qty", 1)), axis=1)
    ...
    df["drawdown_limit"] = calc_trailing_drawdown(df["cum_net_pnl"], buffer=dd_buffer)
    df["day_of_week"]    = df["soldTimestamp"].dt.day_name()
    df["month_label"]    = df["soldTimestamp"].dt.strftime("%b '%y")
    df["trade_num"]      = range(1, len(df) + 1)
    # alias used in legacy chart references
    df["pnl_clean"]      = df["net_pnl"]
    return df


@st.cache_data(show_spinner=False)
def load_csv(data: bytes, starting: float, dd_buffer: float = 2000.0) -> pd.DataFrame:
    raw = pd.read_csv(io.BytesIO(data))
    return process(raw, starting, dd_buffer=dd_buffer)
...
with st.sidebar:
    ...
    drawdown_buffer = st.number_input(
        ...
        min_value=100.0, value=2_000.0, step=100.0, format="%.0f",
        help="e.g. $2,000 for a standard funded account trailing stop.",
    )
...
if uploaded is not None:
    df = load_csv(uploaded.read(), starting_balance, dd_buffer=drawdown_buffer)
    data_source = "live"
elif use_demo:
    df = process(generate_demo(), starting_balance, dd_buffer=drawdown_buffer)
    data_source = "demo"
else:
    df = None
    data_source = "none"
...
with bcol:
    label = "LIVE DATA" if data_source == "live" else "DEMO DATA"
    cls   = "badge-green" if data_source == "live" else "badge-blue"
    st.markdown(
...
if df is None or len(df) == 0:
    ...
    <div style="display:flex;flex-direction:column;align-items:center;
      ...
      <div style="font-family:'DM Sans',sans-serif;font-size:0.9rem;color:#55556a;
        ...
        Upload a CSV or enable <strong style="color:#8888aa;">demo data</strong> to get started.
      </div>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
total_return = (total_net / starting_balance) * 100 if starting_balance else 0.0
...
c1.metric("Current Equity (Net)", f"${curr_equity:,.2f}",
          f"{fmt_usd(total_net, sign=True)}  ({fmt_pct(total_return)})")
c2.metric("Gross P&L", fmt_usd(total_gross, sign=True),
          f"before fees · {total_trades} trades", delta_color="off")
c3.metric("Total Fees", f"-${total_fees:,.2f}",
          f"avg ${total_fees/total_trades:.2f}/trade" if total_trades else "—",
          delta_color="inverse")
c4.metric("Net P&L", fmt_usd(total_net, sign=True),
          f"{len(wins)} wins  /  {len(losses)} losses", delta_color="off")

st.markdown("<br>", unsafe_allow_html=True)

# KPI ROW 2 — Win Rate · Profit Factor · R/R · Max Drawdown
c5, c6, c7, c8 = st.columns(4)
c5.metric("Win Rate", fmt_pct(win_rate), f"{total_trades} total trades", delta_color="off")
...
dd_pct = (max_dd / starting_balance * 100) if starting_balance else 0.0
...
c11.metric("Largest Win Streak",  f"{win_streak} trades",  "consecutive wins",   delta_color="off")
c12.metric("Largest Loss Streak", f"{loss_streak} trades", "consecutive losses", delta_color="off")


# ─────────────────────────────────────────────────────────────────────────────
#  EQUITY CURVE  (Y = Cumulative Net PnL, starts at $0)
# ─────────────────────────────────────────────────────────────────────────────
st.markdown('<div class="section-hdr">Equity Curve — Cumulative Net P&L</div>',
            unsafe_allow_html=True)

# Prepend a $0 origin point
start_pt = pd.DataFrame({
    "soldTimestamp":  [df["soldTimestamp"].iloc[0] - pd.Timedelta(minutes=5)],
    "cum_net_pnl":    [0.0],
    "drawdown_limit": [-float(drawdown_buffer)],
    "trade_num":      [0],
    "net_pnl":        [0.0],
    "fees":           [0.0],
    "gross_pnl":      [0.0],
...
eq_df = pd.concat(
    [start_pt, df[["soldTimestamp", "cum_net_pnl", "drawdown_limit",
...
# Trailing drawdown limit — red dashed line
fig_eq.add_trace(go.Scatter(
    x=eq_df["soldTimestamp"],
...
# Layout — NO yaxis_tickprefix / yaxis_tickformat shorthands;
# all axis config is in explicit xaxis= / yaxis= dicts to avoid collisions.
fig_eq.update_layout(
    **PLOT,
    ...
    legend=dict(
        ...
        font=dict(size=11, family="DM Sans, sans-serif", color="#8888aa"),
    ),
    xaxis=_axis(),
    yaxis=_axis(
        tickprefix="$",
...
if locked:
    dd_color  = "#22c55e"
    lock_note = "🔒 Limit locked at $0 — buffer fully secured"
elif headroom > drawdown_buffer * 0.5:
    dd_color  = "#22c55e"
    lock_note = f"✅  ${headroom:,.2f} headroom remaining"
elif headroom > drawdown_buffer * 0.2:
    dd_color  = "#f59e0b"
    lock_note = f"⚠️  Only ${headroom:,.2f} headroom — approaching limit"
else:
    dd_color  = "#f43f5e"
    lock_note = f"🚨  DANGER — only ${headroom:,.2f} before breach!"
...
st.markdown(
    f"""<div style="background:rgba(244,63,94,0.06);border:1px solid rgba(244,63,94,0.18);
      ...
      <strong>Trailing Drawdown Status:</strong>
      &nbsp;Limit at <strong>${current_dd_limit:,.2f}</strong>
...
# ─────────────────────────────────────────────────────────────────────────────
#  ANALYTICS ROW — Day-of-Week bars + Win/Loss donut
# ─────────────────────────────────────────────────────────────────────────────
...
with left:
    ...
    dow = (
        df.groupby("day_of_week")["net_pnl"]
        .sum()
    ...
    fig_dow.update_layout(
        **PLOT,
        ...
        bargap=0.35,
        xaxis=_axis(),
        yaxis=_axis(tickprefix="$", tickformat=","),
    )
...
with right:
    ...
    # Pie chart: no xaxis/yaxis needed
...
# ─────────────────────────────────────────────────────────────────────────────
#  MONTHLY NET P&L BAR  (only if > 1 month of data)
# ─────────────────────────────────────────────────────────────────────────────
monthly = (
    df.groupby("month_label")["net_pnl"]
    .sum()
    .reset_index()
...
tbl = df[[
    "trade_num", "soldTimestamp", "symbol", "qty",
    "buyPrice", "sellPrice", "gross_pnl", "fees", "net_pnl", "duration",
]].copy()
tbl.columns = ["#", "Date / Time", "Symbol", "Qty",
               "Entry", "Exit", "Gross P&L", "Fees", "Net P&L", "Duration"]
...
tbl["Net P&L"]  = tbl["Net P&L"].apply(lambda v: f"+${v:,.2f}" if v > 0 else f"-${abs(v):,.2f}")
...
pill(p5, "Expectancy/Trade", f"${expectancy:,.2f}")
